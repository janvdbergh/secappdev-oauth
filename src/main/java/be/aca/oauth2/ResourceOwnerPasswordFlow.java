...
public class ResourceOwnerPasswordFlow {
    ...
    public static void main(String[] args) throws Exception {
        ...
        parameters.add(new BasicNameValuePair("client_id", OAuth2Provider.SALESFORCE_CLIENT_ID));
        parameters.add(new BasicNameValuePair("client_secret", OAuth2Provider.SALESFORCE_CLIENT_SECRET));
        parameters.add(new BasicNameValuePair("username", userName));
        parameters.add(new BasicNameValuePair("password", password + securityToken));

        // Call Salesforce
        String response = Util.sendHttpPost(OAuth2Provider.SALESFORCE_TOKEN_URL, parameters);
